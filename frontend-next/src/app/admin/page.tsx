...
export default function AdminPage() {
  ...
  const deleteUser = async (userId: number, username: string) => {
    if (
      !confirm(
        `Are you sure you want to completely delete user @${username}? This deletes all their submissions too!`,
      )
    )
      return;
    try {
  ...
  async function fetchProblems() {
    try {
      const data = await fetchJSON<Problem[]>(
        `${getApiBase()}/api/v1/problems`,
      );
  ...
  const generateWithAI = async () => {
    if (!aiPrompt.trim() && !formData.title) {
      setMessage("Enter a prompt or title first");
      setIsError(true);
      return;
    }
...
- description MUST be properly formatted HTML, not plain text`;
      ...
      if (Array.isArray(generated.test_data)) {
        testDataStr = (generated.test_data as Array<unknown>)
      ...
      setFormData((prev) => ({
        ...
        python_template:
          ((generated.python_template ||
            ...
            generated.python_starter) as string) || prev.python_template,
        cpp_template:
          ((generated.cpp_template ||
            generated.cppTemplate ||
            generated.cpp_starter) as string) || prev.cpp_template,
        driver_python:
          ((generated.driver_python ||
            generated.driverPython ||
            generated.python_driver) as string) || prev.driver_python,
        driver_cpp:
          ((generated.driver_cpp ||
            generated.driverCpp ||
            generated.cpp_driver) as string) || prev.driver_cpp,
  ...
  const testProblem = async () => {
    ...
    try {
      ...
      if (data.exit_code === 0 && output.includes("CASE|")) {
        ...
        if (hasFailures) {
          setMessage("Some cases failed — check output below");
          setIsError(true);
        } else {
        ...
        setMessage("Execution failed — check output below");
        setIsError(true);
      }
    } catch (err) {
  ...
  const easyPct =
    totalProblemCount > 0
      ? Math.round((easyCount / totalProblemCount) * 100)
      : 0;
  const mediumPct =
    totalProblemCount > 0
      ? Math.round((mediumCount / totalProblemCount) * 100)
      : 0;
    ...
    <>
      ...
      <div className="admin-layout">
        ...
        <aside className="sidebar">
          <div className="sidebar-section">
            ...
            <a
              className={`sidebar-link${sidebarView === "dashboard" ? " active" : ""}`}
              href="#"
              onClick={(e) => {
                e.preventDefault();
            ...
            <a
              className={`sidebar-link${sidebarView === "problems" ? " active" : ""}`}
              href="#"
              onClick={(e) => {
                e.preventDefault();
            ...
            <a
              className={`sidebar-link${sidebarView === "users" ? " active" : ""}`}
              href="#"
              onClick={(e) => {
                e.preventDefault();
            ...
            >
              ...
              Users
              {adminStats && (
                <span className="sidebar-count">{adminStats.users.total}</span>
              )}
            </a>
            <a
              className={`sidebar-link${sidebarView === "submissions" ? " active" : ""}`}
              href="#"
              onClick={(e) => {
                e.preventDefault();
              ...
              <div className="metric-grid stagger">
                <div className="metric-card">
                  <div className="metric-top">
                    ...
                    {adminStats && adminStats.users.new_today > 0 && (
                      <span className="metric-change up">
                        +{adminStats.users.new_today} today
                      </span>
                    )}
                  </div>
                ...
                <div className="metric-card">
                  <div className="metric-top">
                    ...
                    {adminStats && (
                      <span className="metric-change up">
                        {adminStats.submissions.today} today
                      </span>
                    )}
                  </div>
                  <div className="metric-value">
                ...
                <tbody>
                  {filteredProblems.map((p, i) => (
                    <tr key={p.slug}>
                      ...
                      <td>
                        <span
                          className={`diff-badge ${diffColor(p.difficulty)}`}
                        >
                          {p.difficulty}
                        </span>
                      </td>
                ...
                <tbody>
                  {adminStats?.recent_users?.map((u) => (
                    <tr key={u.id}>
                      ...
                      <td>
                        <span
                          style={{ color: "var(--diff-easy)", fontWeight: 700 }}
                        >
                          {u.solved}
                        </span>
                      </td>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button
                            ...
                            style={{
                              opacity: u.is_admin ? 1 : 0.5,
                              color: u.is_admin ? "#818cf8" : "inherit",
                            }}
                          >
                            <svg viewBox="0 0 24 24">
                          ...
                          >
                            <svg viewBox="0 0 24 24">
                              <path d="M3 6h18" />
                              <path d="M19 6L17 21a2 2 0 01-2 2H9a2 2 0 01-2-2L5 6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2" />
                            </svg>
                          </button>
                        </div>
                      </td>
            ...
            <>
              <div className="metric-grid stagger">
                ...
                <div className="metric-card">
                  ...
                  >
                    {adminStats?.submissions.pass_count ?? "..."}
                  </div>
                ...
                <div className="metric-card">
                  <div
                    ...
                    style={{ color: "var(--diff-hard)" }}
                  >
                    {adminStats?.submissions.fail_count ?? "..."}
                  </div>
              ...
              {adminStats?.language_distribution && (
                <div className="table-card fade-in-up">
                  ...
                  >
                    {Object.entries(adminStats.language_distribution).map(
                      ([lang, count]) => {
              ...
              {adminStats && adminStats.top_problems.length > 0 && (
                <div className="table-card fade-in-up">
                  <div className="table-header">
                    <h3>Problems by Submission Count</h3>
                  </div>
                  <table className="admin-table">
                    <thead>
                      <tr>
      ...
      {showModal && (
        ...
        >
          ...
          >
            <h2>
              {viewMode === "create" ? "Create New Problem" : "Edit Problem"}
            </h2>
            ...
            >
              <div className="modal-form-group">
                <label style={{ color: "#a78bfa" }}>
                  AI Prompt (describe the problem to generate)
                </label>
                <textarea
                  value={aiPrompt}
